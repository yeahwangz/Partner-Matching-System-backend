...
create definer = root@localhost trigger delete_table_user_tag_forSearch_by_user
    after delete
    on user
    for each row
BEGIN
...
create definer = root@localhost trigger insert_table_user_tag_forSearch_by_user
    after insert
    on user
    for each row
BEGIN
...
create definer = root@localhost trigger update_table_user_tag_forSearch_by_user
    after update
    on user
    for each row
BEGIN
...
-- begin
-- auto-generated definition
create schema knowledgeplanet collate utf8mb4_0900_ai_ci;
...
(
    ...
    tags          varchar(1024)                                 null comment '队伍标签',
    avatarUrl     varchar(1024) default 'src/assets/01geek.jpg' not null comment '队伍头像',
    currentMember json                                          not null comment '当前成员，json数组格式字符串',
    historyMember json                                          not null comment '当前成员，json数组格式字符串',
    historyLeader json                                          not null comment '当前成员，json数组格式字符串',
    maxMemberNum  int           default 5                       not null comment '队伍人数上限',
    isDelete      tinyint       default 0                       not null comment '队伍是否已解散，0-存在，1-已解散',
    createTime    datetime      default CURRENT_TIMESTAMP       null comment '创建时间',
    updateTime    datetime      default CURRENT_TIMESTAMP       null on update CURRENT_TIMESTAMP comment '更新时间',
...
(
    ...
    id       bigint auto_increment comment 'id'
        primary key,
    tags     varchar(1024)     null comment '标签 json 列表',
    isDelete tinyint default 0 not null comment '队伍是否存在,0-存在，1-不存在',
    constraint unique_userAccount
        unique (teamId)
)
    comment '队伍id-标签表';
